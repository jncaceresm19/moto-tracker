...
import { View, Text, StyleSheet, TouchableOpacity, Image, TextInput, Modal, Dimensions, FlatList, KeyboardAvoidingView, Platform, Animated, PanResponder } from 'react-native';
...
export function TheftAlertCard({
  ...
  const isScrollCloseLocked = useRef(false);
  const memberSinceYear = ownerCreatedAt ? new Date(ownerCreatedAt).getFullYear() : null;

  const closeCommentsSheet = () => {
    Animated.timing(sheetTranslateY, { toValue: Dimensions.get('window').height, duration: 200, useNativeDriver: true }).start(() => {
      setShowCommentsSheet(false);
      // Do NOT reset sheetTranslateY here — it causes a flash before Modal fade-out completes
      scrollOffsetY.current = 0;
      isScrollCloseLocked.current = false;
    });
  };
  ...
  // Pill handle PanResponder — drag down to dismiss
  ...
  // FlatList overscroll → close (Instagram behavior)
  const handleCommentsScroll = useCallback((e: any) => {
    const y = e.nativeEvent.contentOffset.y;
    const vy = e.nativeEvent.velocity?.y ?? 0;
    scrollOffsetY.current = y;

    // User scrolled past top while swiping down — close immediately
    if (y < -20 && vy > 0.3 && !isScrollCloseLocked.current) {
      isScrollCloseLocked.current = true;
      closeCommentsSheet();
    }
  }, []);
      ...
      <View style={[styles.card, { backgroundColor: cardBg, borderColor: cardBorder }]}>
        ...
        <View style={[styles.statusLabel, { backgroundColor: isRecovered ? colors.green : colors.alertRed }]}>
          <Text style={styles.statusLabelText}>
            {isRecovered ? 'ENCONTRADA' : 'ALERTA DE ROBO · ACTIVA'}
          </Text>
        </View>
        ...
        <View style={[styles.actionRow, { borderTopColor: colors.border }]}>
          ...
          <TouchableOpacity
            style={[styles.actionBtn, status === 'recovered' && { opacity: 0.3 }]}
            onPress={openCommentsSheet}
            disabled={status === 'recovered'}
          >
            <Ionicons name="chatbubble-outline" size={20} color={status === 'recovered' ? colors.inkFaint : colors.inkSoft} />
            {responses.length > 0 && (
              ...
              <View style={[styles.profileModal, { backgroundColor: colors.surface }]}>
                ...
                <View style={styles.profileContent}>
                  <View style={styles.profilePhotoWrapper}>
                    ...
                    {ownerPlan && (
                      <View style={[styles.planBadge, { borderColor: colors.surface }]}>
                  ...
                  <View style={[styles.memberSincePill, { backgroundColor: colors.surfaceSecondary || colors.surface }]}>
                    <Image source={require('../../assets/nombre.jpeg')} style={styles.profileAppIcon} />
        ...
        {/* Comments Bottom Sheet (Instagram style — pill handle, swipe to dismiss, no X) */}
        <Modal visible={showCommentsSheet} transparent animationType="fade" onRequestClose={closeCommentsSheet}>
          ...
          >
            ...
            />
            <Animated.View
              style={[styles.sheetContainer, { backgroundColor: colors.surface, transform: [{ translateY: sheetTranslateY }] }]}
            >
              {/* Pill handle — drag to dismiss */}
              <View style={styles.sheetHandleBar} {...pillPanResponder.panHandlers}>
                  ...
                  renderItem={({ item: comment }) => (
                    <View style={[styles.sheetComment, { borderBottomColor: colors.borderLight }]}>
                      ...
                      <View style={styles.sheetCommentContent}>
                        <View style={styles.sheetCommentNameRow}>
              ...
              {status === 'active' ? (
                ...
                <View style={[styles.sheetInputRow, { borderTopColor: colors.border, backgroundColor: colors.surface, opacity: 0.5 }]}>
                  <Text style={[styles.sheetInput, { color: colors.inkFaint, textAlign: 'center', borderWidth: 0, fontSize: 13 }]}>
...
const styles = StyleSheet.create({
  ...
  menuBtn: { padding: 4 },
  statusLabel: { alignSelf: 'flex-start', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 30, marginHorizontal: 14, marginTop: 10 },
  ...
  profilePhotoPlaceholder: { width: 100, height: 100, borderRadius: 50, justifyContent: 'center', alignItems: 'center' },
  ...
  sheetContainer: { maxHeight: Dimensions.get('window').height * 0.8, borderTopLeftRadius: 16, borderTopRightRadius: 16, overflow: 'hidden' },
  sheetHandleBar: { paddingVertical: 10, alignItems: 'center' },
  ...
  sheetCommentNameRow: { flexDirection: 'row', alignItems: 'center', gap: 4 },
