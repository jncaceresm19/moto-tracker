  ...
  return (
    <SafeAreaView style={[styles.screen, { backgroundColor: colors.background }]} edges={[]}>
      ...
      <View style={styles.emptyState}>
        <Ionicons name="diamond-outline" size={48} color={colors.inkFaint} />
        <Text style={[styles.emptyTitle, { color: colors.ink }]}>{t('noSubscriptions')}</Text>
        <Text style={[styles.emptyText, { color: colors.inkFaint }]}>{t('noSubscriptionsText')}</Text>
      </View>
    </SafeAreaView>
  );
}
