...
// Simple email sender (placeholder — replace with real SMTP in production)
async function sendEmail(to: string, subject: string, body: string): Promise<void> {
  console.log(`[OTP] Sending to ${to}: ${subject} — ${body}`);
...
router.post('/send', authenticate, validateBody(sendOtpSchema), async (req: Request, res: Response) => {
  try {
    ...
    const result = await sendOtp(userId, email, sendEmail);
